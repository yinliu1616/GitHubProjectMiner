...
public class ASTFieldDeclare {
	...
	public void persistASTFieldDeclare(MySQLConnection db) {
		...
		try {
			preparedStatement = db.getConn().prepareStatement("insert into "+ ASTFIELDDECLARE_TABLE +"(localfile_path,project_url,commit_sha1,type,name,initializer,modifiers) values ( ?, ?, ?, ?, ?, ?, ?);");
		    ...
		    if (initializer!=null && initializer.length()>MAX_INITIALIZER){
		    	preparedStatement.setString(6, initializer.substring(0, MAX_INITIALIZER));
		    }else {
